...
$sql = "SELECT `ID`, `Name`, `Email`, `Contact`, `Specialization`, `Experience` FROM `Doctor`";
...
<head>
	...
	<title>Doctor List - NeuCare Admin Dashboard</title>
							...
							<table class="table table-striped" id="table1">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<?= strval($doctor["Contact"]) ?>
										</td>
										<td>
											<?= strval($doctor["Specialization"]) ?>
										</td>
										<td>
											<?= strval($doctor["Experience"]) ?>
										</td>
										<td>
											<span style="cursor: pointer;" class="badge bg-danger"
												onclick="deleteDoctor(<?= intval($doctor['ID']) ?>, this)">Remove</span>
										</td>
									</tr>
									<?php endwhile; ?>
		...
		function deleteDoctor(id, el) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					fetch("handle-edit-delete.php?delete-doctor=" + id)
						.then(res => {
							if (res.status == 204) {
								Toastify({
									text: "Doctor Removed",
