...
$sql = "SELECT `ID`, `Name`, `Email`, `Contact`, `Gender`, `DOB` FROM `Patient`";
...
<head>
	...
	<title>Patients - NeuCare Admin Dashboard</title>
							...
							<table class="table table-striped" id="table1">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<?= strval($patient["Email"]) ?>
										</td>
										<td>
											<?= strval($patient["Contact"]) ?>
										</td>
										<td>
											<?= strval($patient["Gender"]) ?>
										</td>
										<td>
											<?= strval($patient["DOB"]) ?>
										</td>
										<td>
											<span style="cursor: pointer;" class="badge bg-danger"
												onclick="deletePatient(<?= intval($patient['ID']) ?>, this)">Remove</span>
										</td>
									</tr>
									<?php endwhile; ?>
		...
		function deletePatient(id, el) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					fetch("handle-edit-delete.php?delete-patient=" + id)
						.then(res => {
							if (res.status == 204) {
								Toastify({
